...
class PDFType0Font : PDFFont {
    ...
    override func pdfStringToCharacterIds(_ pdfString: CGPDFStringRef) -> [PDFFontFile.CharacterId] {
        ...
        let unicharArray = originalCharCodes.withUnsafeBytes { body -> [UInt16] in
            var res = [UInt16]()

            let value = body.load(as: UInt16.self)
            res.append(currentSystemIsLittleEndian ? value.byteSwapped : value)
