...
public class Parser {
    ...
    public init(document: CGPDFDocument, pageNumber: Int, delegate: ParserDelegate, indexer: DocumentIndexer, log: Bool = false) throws {
...
//        for p in 1...2 {
            ...
            indexer.beginNewPage(pageNumber: pageNumber, pageSize: rect.size)
            parsePage(page, operators: operatorTableRef, selfPtr:selfPointer)
            delegate.parser(p: self, didParse: pageNumber, outOf: document.numberOfPages)
//        }
        delegate.parser(p: self, didCompleteWithError: nil, cgPDFDocument: document)
    }
