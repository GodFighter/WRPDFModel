...
public class SimpleDocumentIndexer {
    ...
    public class PageIndex {
        ...
        func lineDescription(_ line: Int) -> String {
            let sortedLines = lines.keys.sorted().map { (key) -> LineTextBlock in
                return lines[key]!
            }
            let textBlocks = Array(sortedLines)
            let lineCount = textBlocks.count
            if line >= lineCount {
                return ""
            }

            let textBlock = textBlocks[line]
    ...
    public struct LineTextBlock {
        ...
        func wordCoalesced() -> LineTextBlock {
            ...
            var iNext = 1
            let sortedBlocks = blocks.sorted(by: {
